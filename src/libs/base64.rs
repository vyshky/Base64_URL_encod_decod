pub fn base64encode(input: &str) -> String
{
    ...
    for i in (0..input_length).step_by(3)
    {
        ...
        base64_chars[0] = (char_array_3[0] & 0xFC) >> 2;
        ...
        base64_chars[1] = ((char_array_3[0] & 0x03) << 4) + ((char_array_3[1] & 0xF0) >> 4);
        ...
        base64_chars[2] = ((char_array_3[1] & 0x0F) << 2) + ((char_array_3[2] & 0xC0) >> 6);
        // Тут берем последние 6 битов и сразу же записываем
        base64_chars[3] = char_array_3[2] & 0x3F;
        ...
        for j in 0..4 {
            if (i + j <= input_length) {
                encoded_string.push(char::from(base64_chars.as_bytes()[base64_chars[j] as usize]));
            } else {
                encoded_string.push('=');
            }
        }
